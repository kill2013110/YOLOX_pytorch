...
class Exp(MyExp):
    def __init__(self):
        ...
        self.exp_name = f'{os.path.split(os.path.realpath(__file__))[1].split(".")[0]}_{self.head_type}' \
                        f'_{self.get_face_pionts}points_{self.aug_epochs}straug_{self.max_epoch-self.min_lr_epochs}coslr_{self.points_loss_weight}__greater0.9'
                        # f'_{self.var_config}'
                        # f'test'
        ...
        # self.train_ann = r"F:\datasets\Diverse_Masked_Faces_v2_m\ann/train_v3_small.json"
        # self.val_ann = r"F:\datasets\Diverse_Masked_Faces_v2_m\ann/val_v3_small.json"
