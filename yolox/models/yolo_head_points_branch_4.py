...
class YOLOXHead_points_branch_4(nn.Module):
    def __init__(
        self,
        num_classes,
        width=1.0,
        ...
        # if points_branch==1:
        self.points_preds_reg = nn.ModuleList()
        self.points_preds_cls = nn.ModuleList()
    ...
    def forward(self, xin, labels=None, imgs=None):
        ...
        for k, (cls_conv, reg_conv, stride_this_level, x, points_conv) in enumerate(
            ...
            points_reg_feat = reg_feat
            points_cls_feat = cls_feat
            points_reg_output = self.points_preds_reg[k](points_reg_feat)
            points_cls_output = self.points_preds_cls[k](points_cls_feat)
            # points_output = (points_reg_output + points_cls_output)/2
            reg_output = torch.cat([reg_output, points_reg_output, points_cls_output], 1)
            if self.training:
    ...
    def get_output_and_grid(self, output, k, stride, dtype):
        ...
        n_ch = 4+ 2*2*self.get_face_pionts +1 + self.num_classes if self.get_face_pionts else 5 + self.num_classes
        ...
        if self.get_face_pionts:#outputs: [xc,yc,w,h, [x,y]*6 ,obj, cls...]
            for i in range(4, 4+self.get_face_pionts*2*2, 2):
    ...
    def decode_outputs(self, outputs, dtype):
        ...
        if self.get_face_pionts:#outputs: [xc,yc,w,h, [x,y]*6 ,obj, cls...]
            ...
            outputs[..., 4:4+2*self.get_face_pionts] = (outputs[..., 4+2*self.get_face_pionts]+outputs[...,4+2*self.get_face_pionts:4+4*self.get_face_pionts])/2
        return outputs
        # return org_outputs
